...
export async function buildAccounts(store: DnaDataStore): Promise<AccountsViewModel> {
  ...
  const accountOpps = new Map<string, AccountOpportunity[]>()
  ...
  for (const link of allLinks) {
    ...
    if (from.typeName === 'account' && to.typeName === 'contact') {
      ...
      if (name) accountOwner.set(from.id, name)
    }
  ...
  const result: AccountEntry[] = accounts.map(acc => ({
    id: acc.id,

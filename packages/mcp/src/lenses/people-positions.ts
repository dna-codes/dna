...
export async function buildPeoplePositions(store: DnaDataStore): Promise<PeoplePositionsViewModel> {
  ...
  const positionToPersonMap = new Map<string, string>()
  for (const link of allLinks) {
    if (link.from.typeName === personTypeName && link.to.typeName === positionTypeName) {
      const personName = personById.get(link.from.id)
      if (personName) positionToPersonMap.set(link.to.id, personName)
    }
  }

  const positions: PositionEntry[] = positionInstances.map(pos => ({
    id: pos.id,

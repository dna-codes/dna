...
export async function buildJobDescriptions(store: DnaDataStore): Promise<JobDescriptionsViewModel> {
  ...
  const allInstances = new Map<string, { id: string; name: string; description?: string; typeName: string }>()
  await Promise.all(
    typeNames.map(async (tn) => {
      ...
      for (const inst of instances) {
        allInstances.set(inst.id, {
          ...
          description: (inst as Record<string, unknown>).description as string | undefined,
          typeName: tn,
        })
      }
    }),
  ...
  const posReportsTo = new Map<string, string>()
  if (reportsToRel) {
    for (const link of allLinks) {
  ...
  const posDept = new Map<string, string>()
  for (const link of allLinks) {
  ...
  const stepsByPos = new Map<string, JobDescResponsibility[]>()
  if (assignedToRel) {
    for (const link of allLinks) {

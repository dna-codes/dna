...
describe('query_instances', () => {
  it('filters by type', async () => {
    ...
    expect(positions.map((p: Record<string, unknown>) => p.name)).toContain('CEO')
  ...
  it('matches nameContains case-insensitively', async () => {
    ...
    const matched = (all as Array<Record<string, unknown>>).filter(i =>
      String(i.name ?? '').toLowerCase().includes('operations')
    )
...
describe('get_links', () => {
  ...
  it('filters links by relationship type name', async () => {
    ...
    const filtered = allLinks.filter(
      l => l.from.typeName === fillsRelType.from && l.to.typeName === fillsRelType.to
    )
...
describe('patch_graph validation', () => {
  it('accepts a valid add_instance op', async () => {
    ...
    const all = await store.instance.list('position')
    expect((all as Array<Record<string, unknown>>).map(i => i.name)).toContain('COO')
  })
...
describe('get_lens: org-chart', () => {
  it('returns a valid org-chart view-model', async () => {
    ...
    const viewModel = await buildOrgChart(store)
...
describe('auth middleware', () => {
  it('pass-through middleware calls next immediately', () => {
    const passthroughAuth = (_req: unknown, _res: unknown, next: () => void) => next()

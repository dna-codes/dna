...
async function seedOrg(store) {
    ...
    const link = (from, to, role) => store.link.create(from, to, { role });
...
function stripIds(node) {
    const { id: _id, parentId: _p, holders, reports, ...rest } = node;
    return {
        ...rest,
...
describe('org-chart regression', () => {
    it('produces a stable org-chart view-model', async () => {
        ...
        await seedOrg(store);
        const vm = await (0, org_chart_js_1.buildOrgChart)(store);
        const stable = { lens: vm.lens, groupName: vm.groupName, roots: vm.roots.map(r => stripIds(r)) };
        expect(stable).toMatchSnapshot();
    });
});
//# sourceMappingURL=org-chart-regression.test.js.map
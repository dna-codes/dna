...
export const MERGE_TYPEDEF_CYPHER =
  'MERGE (n:TypeDefinition {name: $name}) SET n += $props'

/** Cypher to MERGE a RelationshipDef node. Properties passed via `$props`. */
export const MERGE_RELDEF_CYPHER =
  'MERGE (n:RelationshipDef {name: $name}) SET n += $props'

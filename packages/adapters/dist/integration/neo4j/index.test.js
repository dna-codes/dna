...
const LENDING_FIXTURE_PATH = (0, path_1.join)(__dirname, '..', '..', '..', '..', '..', 'examples', 'lending', 'operational.json');
function envOptions() {
    return {
...
function lendingDna() {
    return JSON.parse((0, fs_1.readFileSync)(LENDING_FIXTURE_PATH, 'utf-8'));
}
function registryDna() {
    return JSON.parse((0, fs_1.readFileSync)(REGISTRY_FIXTURE_PATH, 'utf-8'));
}
...
describeNeo4j('integration/neo4j live', () => {
    ...
    let createdLinks = [];
    beforeAll(async () => {
        client = (0, client_1.createClient)(envOptions(), lendingDna());
    ...
    it('caller-provided id collision throws', async () => {
        ...
        await expect(client.instance.create('Loan', { id: fixed, amount: 2 })).rejects.toThrow(/already exists/);
    });
    it('Link create + list + delete round-trip with role and attributes', async () => {
        ...
        createdInstances.push({ typeName: 'Loan', id: loan.id }, { typeName: 'Person', id: borrower.id });
        const link = await client.link.create({ typeName: 'Loan', id: loan.id }, { typeName: 'Person', id: borrower.id }, { role: 'primary_borrower', attributes: { assigned_at: '2026-05-23' } });
...
describeNeo4j('integration/neo4j registry fixture round-trip', () => {
    it('migrates a registry DNA and round-trips TypeDefinition + Instance + Link', async () => {
        const client = (0, client_1.createClient)(envOptions(), registryDna());
        try {
            await client.migrate();
            ...
            const link = await client.link.create({ typeName: 'Instance', id: i1.id }, { typeName: 'Instance', id: i2.id }, { role: 'belongs_to' });
...
describeNeo4j('integration/neo4j clear()', () => {
    it('wipes the whole graph, drops the seed marker, and stays usable', async () => {
        const client = (0, client_1.createClient)(envOptions(), lendingDna());
        try {
            await client.migrate();

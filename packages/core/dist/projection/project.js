...
function project(business, opts = {}) {
    const byId = new Map();
    ...
    const neighborsOfType = (id, type) => [...(adj.get(id) ?? [])].map(n => byId.get(n)).filter((n) => !!n && n.type === type);
    const nodesByKey = new Map();
    const edges = [];
    const key = (realizes, level, parent) => `${level}:${realizes}:${parent ?? 'root'}`;
    const levelFor = (id, def) => opts.levelOverrides?.[id] ?? def;
    const emit = (node) => { if (!nodesByKey.has(node.key))
        nodesByKey.set(node.key, node); };
    const edge = (from, to, via) => { edges.push({ from, to, via }); };

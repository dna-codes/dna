...
exports.PERMISSION_TYPE_NAME = exports.PRODUCT_LEVEL_TYPE_NAME = void 0;
...
function productResourceSchemas() {
    const ui = index_1.schemas.product.ui;
    const api = index_1.schemas.product.api;
    const web = index_1.schemas.product.web;
    const core = index_1.schemas.product.core;
 ...
 */
const GOVERNANCE_RELATIONSHIP_TYPES = [
    {
        name: 'can_access',
        from: '*', // Role | User
        ...
        description: 'Authored governance grant: a Role (or User, for direct grants) may access a structural product surface (App/Module/Workflow/Page). The coarse, structural-level access grant — distinct from operation-level permission. Never derived; preserved across projection re-runs.',
    },
    {
        name: 'assigned_to',
        from: '*', // User
        ...
        description: 'Authored governance edge: a User is provisioned into (homed in) an App. Distinct from can_access — a user may can_access many apps while being assigned_to one. Never derived.',
    },
    {
        name: 'grants',
 ...
 */
async function seedProductTypes(store) {
    const resourceTypes = productResourceSchemas().map((schema) => ({
    ...
    await Promise.all(resourceTypes.filter((rt) => !existingRtNames.has(rt.name)).map((rt) => store.resourceType.create(rt)));
    await Promise.all([...STRUCTURAL_RELATIONSHIP_TYPES, ...GOVERNANCE_RELATIONSHIP_TYPES]
...
exports.PRODUCT_LEVEL_TYPE_NAME = {
    app: 'App',
    module: 'Module',
    workflow: 'Workflow',
    page: 'Page',
...
const GOVERNANCE_ROLES = GOVERNANCE_RELATIONSHIP_TYPES.map((rt) => rt.name);
/**
 * Persist `subgraph` into `store`. See the module header for the full contract.
 * Returns a per-run tally of what changed.
 */
async function applyProjection(subgraph, store) {
    const report = { instancesCreated: 0, linksCreated: 0, orphaned: 0 };
    // 1. Upsert each node as an instance keyed by its stable projection key.
    const keyToRef = new Map();
    for (const node of subgraph.nodes) {
        const typeName = exports.PRODUCT_LEVEL_TYPE_NAME[node.level];
        if (!typeName)
            continue;
    ...
    for (const edge of subgraph.edges) {
        ...
        if (!toRef)
            continue;
        if (await ensureLink(store, fromRef, toRef, edge.via))
            report.linksCreated++;
    }
    ...
    for (const typeName of new Set(Object.values(exports.PRODUCT_LEVEL_TYPE_NAME))) {
        for (const rec of await store.instance.list(typeName)) {
            const key = rec._projectionKey;
            if (typeof key !== 'string' || liveKeys.has(key) || rec._orphaned === true)
                continue;
...
async function findByProjectionKey(store, typeName, key) {
...
async function hasGovernanceEdge(store, ref) {
...
exports.PERMISSION_TYPE_NAME = 'Permission';
/**
 ...
 */
async function applyPermissions(proj, store) {
    const report = { permissionsCreated: 0, grantsCreated: 0 };
    const keyToRef = new Map();
    // 1. Upsert Permissions by {principal, role, scope} identity.
    const existing = await store.instance.list(exports.PERMISSION_TYPE_NAME);
    const identityOf = (r) => `${String(r.principal ?? '')}::${String(r.role ?? '')}::${String(r.scope ?? '')}`;
    const byIdentity = new Map(existing.map((r) => [identityOf(r), r]));
    for (const p of proj.permissions) {
        const hit = byIdentity.get(p.key);
        if (hit) {
            keyToRef.set(p.key, { typeName: exports.PERMISSION_TYPE_NAME, id: hit.id });
            continue;
        }
        const { id } = await store.instance.create(exports.PERMISSION_TYPE_NAME, {
            name: p.key,
        ...
        keyToRef.set(p.key, { typeName: exports.PERMISSION_TYPE_NAME, id });
    ...
    for (const g of proj.grants) {
        ...
        if (!memRef)
            continue;
        if (await ensureLink(store, memRef, permRef, 'grants'))
 ...
 */
async function resolveMembershipRef(store, name) {
    ...
    for (const type of ordered) {
        ...
        if (hit)
            return { typeName: type.name, id: hit.id };
    }
    return null;
}

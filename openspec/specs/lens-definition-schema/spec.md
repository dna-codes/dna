# lens-definition-schema Specification

## Purpose

Defines the declarative shape a lens conforms to: a single unified definition (one meta-schema) distinguished by `target: "schema" | "data"`, with slots/edges that carry free or pinned bindings and an optional grouping `scope`. The shape is a backward-compatible superset of the original `{ name, nodes, edges, sentence }`, so existing all-free lens files validate unchanged.

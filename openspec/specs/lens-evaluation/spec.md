# lens-evaluation Specification

## Purpose

A runtime that evaluates a lens definition against a `DnaDataStore` and returns the matched result — a subgraph `{ nodes, links }` for a data lens, or `{ resourceTypes, relationshipTypes }` for a schema lens. It replaces per-lens hand-coded traversal: free bindings match any instance of a type, pinned bindings seed from a specific instance, and a `scope` expands a grouping from its anchor. Evaluation is separate from presentation — the evaluator returns data only.

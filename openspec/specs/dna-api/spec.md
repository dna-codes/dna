...
Defines `@dna-codes/dna-api`, a deployable GraphQL API server that loads an `OperationalDNA` at startup and generates a GraphQL schema from it. DNA noun primitives become typed GraphQL types; attributes become fields; relationships become expansion fields with cardinality semantics; DNA operations become domain-specific mutations alongside type-generic CRUD. Every resolver routes through an injected `DnaDataStore` contract; the package imports no backend driver directly. Tests inject `integration/memory`; production code injects `integration/neo4j`. The package ships a CLI and Dockerfile + compose assets for local-dev deployment.
## Requirements

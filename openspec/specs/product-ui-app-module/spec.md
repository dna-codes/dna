# product-ui-app-module Specification

## Purpose

Gives the Product UI hierarchy its missing top rungs — `App` and `Module` as first-class graph resource types — and the relationships that bind a product surface to the business it realizes. An `App` is the product face of a grouping anchor (a function/Domain); a `Module` is a feature-area surface beneath it. The `realized_as` edge links any product node to the business node it surfaces (with the level a per-node binding), and `exposes` links an API `Endpoint` to the `Operation` it surfaces. The API surface reuses the existing `Endpoint` and `Namespace` primitives rather than introducing duplicates.
...
This capability SHALL NOT introduce new API primitives. The existing `product/api/endpoint` (which names the `Operation` it maps from via its `operation` field) and `product/api/namespace` (which groups resources and maps from a Domain) SHALL serve as the API surface and its grouping. The graph model SHALL connect them via the `exposes` and `realized_as` edges rather than via new node types.

#### Scenario: No new API node types are registered

- **WHEN** `availableSchemas()` is read
- **THEN** it SHALL NOT contain a `product/api/service` schema (the existing `product/api/namespace` is the API grouping)
...
A `realized_as` relationship type SHALL be expressed in the product graph model (as a lens edge), directed from a Product UI/API node to the business node it surfaces. Valid endpoints SHALL include `App → Domain|Group` (any grouping anchor), `Module → Domain`, `Module → Process`, `Workflow → Process`, `Page → Process`, `Page → Task`, `Section → Task`, `Component → Operation`, and `Namespace → Domain`. The level is a per-node binding: the same business node MAY be realized at different levels by different product nodes.

#### Scenario: realized_as edge is traversable both ways

- **WHEN** a `Page` declares `realizes: "CollectApplication"` (a Task)
- **THEN** the graph SHALL represent a `realized_as` edge from the Page to the Task

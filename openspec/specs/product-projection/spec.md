# product-projection Specification

## Purpose

The pure derivation `project(businessSubgraph) → productSubgraph` — a deterministic, store-free function that walks an evaluated business subgraph by node type (`Domain→Process→Task→Operation`) and derives the product (UI + API) nodes it implies, with stable identity keys and a `planned` state wherever the forward business backing is missing. Persistence (`apply`), runtime UI-type registration, and the core↔pack relationship-vocabulary bridge are deferred follow-ups.

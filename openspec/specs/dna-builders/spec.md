# dna-builders Specification

## Purpose

A canonical, pure, types-aware builder API in `@dna-codes/dna-core` for programmatically composing Operational DNA documents. Covers every Operational primitive (Resource, Person, Role, Group, Membership, Operation, Trigger, Rule, Task, Process) plus the top-level DNA shell. Replaces ad-hoc assembly in `merge()`, `input-json`, and `input-text/layered/constructor`. Compile-time + runtime schema enforcement.

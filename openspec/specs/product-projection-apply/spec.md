# product-projection-apply Specification

## Purpose

`applyProjection(subgraph, store)` — idempotent persistence of a projected product subgraph into a `DnaDataStore`. It upserts product instances by the projection's stable identity (create when new, leave when present, no duplicates) and creates the structural `contains`/`realized_as`/`exposes` links, reconciling only those edges. Authored governance edges (e.g. `can_access`, `assigned_to`) are preserved across re-apply, and product nodes whose business backing has vanished are soft-deleted (marked orphaned) rather than hard-deleted.

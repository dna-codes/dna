...
Defines the `@dna-codes/dna-adapters/integration/neo4j` subpath: a Neo4j-backed implementation of the `DnaDataStore` contract from `@dna-codes/dna-core`. Persists the *runtime data* described by an `OperationalDNA` (Loan records, Borrower records, the Links between them) using the registry triad shape demonstrated in `examples/registry`. Instances become labeled nodes (label = Resource/Person/Role/Group name), Links become typed edges between Instance nodes, and `migrate()` seeds TypeDefinition and RelationshipDef metadata nodes from the constructor DNA. Deliberately DNA-aware as a documented exception to the integration pure-I/O rule.

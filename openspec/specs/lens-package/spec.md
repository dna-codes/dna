# lens-package Specification

## Purpose
Defines the `packages/lenses/` npm package (`@dna-codes/dna-lenses`) — its structure, namespace conventions, base schema contract, and registration within `@dna-codes/dna-core`. The lens package is a peer to `packages/schemas/` and carries no runtime dependency on it.

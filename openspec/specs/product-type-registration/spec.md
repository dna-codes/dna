# product-type-registration Specification

## Purpose

Registering the product UI/API resource and structural relationship types in a `DnaDataStore` at runtime (derived from the registered `product/ui/*` schemas), so product instances can be created. `seedProductTypes(store)` mirrors how `seedPack` creates types and is idempotent — safe to re-run.

# type-registry-lens Specification

## Purpose

Build mode is type-focused, so its lenses render the **type registry** (the grammar) rather than instances. A single type-level view-model — resource types as nodes, relationship types as directed edges, carrying stability and attribute metadata — is exposed over REST and consumed by four Build-mode lens panels (schema graph, type org-chart, type reporting-chains, type definition cards), so a modeler can see the grammar they are shaping even when no instances exist.
...
- **WHEN** the registry has resource types but no concrete `belongs_to` relationship types

# lens-primitive Specification

## Purpose
Defines the Lens primitive — a named, reusable graph pattern consisting of typed node slots and directed edge declarations. A Lens governs both query (find matching subgraphs) and command (assert a binding) directions from a single definition.

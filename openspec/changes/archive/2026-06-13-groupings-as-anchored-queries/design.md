...
## Remaining open question — RESOLVED downstream
...
- **One schema or two:** do free and pinned bindings live in the *same* lens schema (a slot
  ...
  legibility? **Resolved by `runtime-lens-mechanism`: one unified lens definition, with a
  `target: "schema" | "data"` discriminator.** A slot is free (`{ slot, type }`) or pinned
  (adds `ref: { id } | { select }`); pinning is permitted only on `target: "data"`. The
  unified shape is a backward-compatible superset of the original `{ name, nodes, edges,
  sentence }`. Rationale: one evaluator, one validator, one mental model — groupings and
  rendering views are the same object at different binding tightness. See the
  `lens-definition-schema` and `lens-evaluation` specs.

## Follow-on disposition

The three "explore/propose separately" follow-ons (tasks 3.1–3.3) are tracked as their own
changes, consistent with this change being doctrine-only (no code):

- **3.1 — runtime lens evaluation mechanism + one-schema-vs-two.** ✅ Shipped as
  `runtime-lens-mechanism` (archived `2026-06-13`). It defines free/pinned bindings, the
  per-anchor `scope` traversal, and the unified `target`-discriminated lens schema; a grouping
  is a `target: "data"` lens with a pinned anchor. Specs: `lens-definition-schema`,
  `lens-evaluation`.
- **3.2 — schema migration (drop `domain.json` containment arrays; demote `path` to a derived
  cache).** ⏳ Still pending — `operational/domain.json` today retains `resources` / `persons`
  / `roles` / `groups` containment arrays and an authoritative `path`. Per this change's
  Non-Goals the migration is deliberately a *separate* change; the direction is captured under
  "What this implies for the existing schemas" above. To be carried by a dedicated change
  (working name `domain-home-edge-migration`); not yet proposed.
- **3.3 — schema-folder / starter-pack layout as pure vocabulary distribution.** ✅ Reframed
  and shipped via the `starter-pack-registry` spec and `derive-agent-pack-contract` (archived
  `2026-06-13`): a "pack" is a registry-level vocabulary bundle, fully decoupled from runtime
  groupings (Decision 1).

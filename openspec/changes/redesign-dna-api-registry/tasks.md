...
- [ ] 8.1 Confirm bumps applied: `dna-core@0.9.0`, `dna-adapters@0.9.0`, `dna-api@0.2.0`
- [ ] 8.2 Smoke build: `npm run build --workspaces --if-present` succeeds across all four packages
- [ ] 8.3 Smoke test: full workspace `npm test` passes (memory tests will cover everything except live Neo4j; that gate continues to skip when `NEO4J_URI` is unset)
- [ ] 8.4 Pause before tagging — confirm with user that release is wanted

...
function findOperation(operationName, operations) {
    return operations.find(o => o.name === operationName);
}
function resolveEffectSet(set, entityVarName) {
...
function generateInterface(resource) {
...
function buildComment(operationName, rules, operation) {
    const lines = [`  // Operation: ${operationName}`];
    ...
    if (!accessRule && !conditionRule && !operation?.changes?.length)
        return lines;
    if (accessRule) {
...
function methodSignature(ep, resource) {
    ...
    const params = [
        ...
        ...(queryParams.length
            ...
            : []),
        ...(hasDtoBody ? [`dto: ${(0, dto_1.dtoClassName)(action, resource)}`] : []),
    ];
...
function buildListBody(ep, resource) {
    const entityFieldNames = new Set(resource.fields.map(f => f.name));
    const filterParams = (ep.params ?? []).filter(p => p.in === 'query' && p.name !== 'page' && p.name !== 'limit' && entityFieldNames.has(p.name));
    ...
    lines.push(`    const page = parseInt(filters.page ?? '1', 10)`, `    const limit = parseInt(filters.limit ?? '20', 10)`, `    const start = (page - 1) * limit`, `    return { data: items.slice(start, start + limit), total: items.length }`);
    return lines;
}
function buildUpdateBody(ep, resource, operation) {
    ...
    if (operation?.changes?.length) {
        for (const ch of operation.changes) {
            ...
            const val = resolveEffectSet(ch.set, varName);
            if (val !== null)
                lines.push(`    updates.${attr} = ${val}`);
        }
    }
    lines.push(`    const updated = { ...${varName}, ...updates }`, `    store.set(id, updated)`);
    lines.push(`    return updated`);
    return lines;
}
function generateService(resource, endpoints, operations, rules) {

...
exports.generateResourceSchemas = generateResourceSchemas;
exports.generateSchemasInit = generateSchemasInit;
const naming_1 = require("./naming");
function fieldLine(field, forRequest) {
    const pyType = (0, naming_1.toPythonType)(field.type);
...
function needsDateImport(fields) {
    return fields.some(f => f.type === 'date' || f.type === 'datetime');
}
function generateResourceSchemas(resource, endpoints) {
    ...
    const responseFields = allFields.map(f => {
        const pyType = (0, naming_1.toPythonType)(f.type);
        return f.name === 'id'
            ? `    id: str`
...
function generateSchemasInit(resources) {
    const imports = resources.map(r => `from app.schemas.${(0, naming_1.toSnakeCase)(r.name)} import *`);
    return `${imports.join('\n')}\n`;
}

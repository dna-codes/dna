...
function extractNestJsSurface(dir: string): Surface {
  ...
  const api = JSON.parse(
    fs.readFileSync(path.join(repoRoot, 'dna/engine/fixtures/dna/lending/product.api.json'), 'utf-8'),
  )
  const core = JSON.parse(
    fs.readFileSync(path.join(repoRoot, 'dna/engine/fixtures/dna/lending/product.core.json'), 'utf-8'),
  )

 ...
 */
const terraform_aws_1 = require("./terraform-aws");
function basePlan(overrides = {}) {
    return {
        domain: 'lending',
        environment: 'prod',
        paths: { domain: 'lending', root: '/tmp/none', dna: '/tmp/none/dna' },
        constructs: [],
        cells: [],
        variables: [],
        ...
        cells: [
            {
                ...
                constructs: [],
                variables: [{ name: 'DATABASE_URL', source: 'secret', required: true }],
                outputs: [],
                outputDir: '/tmp/none/output/api',
            },
            {
...
describe('terraform-aws — mixed plan (lambda api + static UI + db)', () => {
    ...
    test('lambda cell + static UI + db all show up in resources list', () => {
        const result = (0, terraform_aws_1.generateTerraformAws)(mixedPlan);
        expect(result.resources).toEqual(expect.arrayContaining(['lambda:api', 'cloudfront:web', 'cloudfront:api', 'waf:api']));
    });
});
//# sourceMappingURL=terraform-aws.test.js.map
 ...
 */
function runStatus(argv, args) {
    const opts = { json: (0, args_1.boolFlag)(args, 'json') };
    if ((0, args_1.boolFlag)(args, 'help')) {
        console.log(help_1.STATUS_HELP);
        return;
    }
    const [domain] = argv;
    if (!domain) {
        (0, output_1.emitError)('Usage: cba status <domain> --env <environment> [--adapter <name>]', opts);
        process.exit(1);
    }
    ...
    (0, registry_1.statusWith)(adapterId, ctx)

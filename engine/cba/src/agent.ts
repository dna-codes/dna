...
function concernFor(file: string, root: string): string {
  ...
  if (legacyCellMatch) return `cell:${legacyCellMatch[1]}`
  const engineCellMatch = rel.match(/^dna\/engine\/cells-([^/]+)\/AGENTS\.md$/)
  if (engineCellMatch) return `cell:${engineCellMatch[1]}-cell`
...
function findAllAgentsFiles(root: string): AgentContract[] {
  ...
  if (fs.existsSync(engineDir)) {
    for (const entry of fs.readdirSync(engineDir, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name))) {
      if (!entry.isDirectory() || !entry.name.startsWith('cells-')) continue

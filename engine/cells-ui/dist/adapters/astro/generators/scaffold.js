...
exports.generateAstroConfig = generateAstroConfig;
exports.generateGitignore = generateGitignore;
exports.generateLayout = generateLayout;
exports.generatePage = generatePage;
exports.generateBlockComponent = generateBlockComponent;
exports.generateStarlightContentConfig = generateStarlightContentConfig;
exports.generateStarlightContentEntry = generateStarlightContentEntry;
exports.generateStarlightSidebar = generateStarlightSidebar;
/**
 ...
 */
function generatePackageJson(appName, flavor) {
    const baseDeps = {
        astro: '^4.16.0',
    };
    const starlightDeps = {
    ...
    const dependencies = flavor === 'starlight' ? { ...baseDeps, ...starlightDeps } : baseDeps;
 ...
 *     via cell config
 */
function generateAstroConfig(flavor, config, sidebar) {
    if (flavor === 'marketing') {
 ...
 */
function generateLayout(siteTitle, layout) {
 ...
 */
function generatePage(page, route) {
    const importLines = ["import Site from '../layouts/Site.astro'"];
    const seen = new Set();
    const renderLines = [];
 ...
 */
function generateBlockComponent(block) {
        ...
        switch (block.type) {
            ...
            case 'detail':
                return `    <dl class="block-detail">\n${(block.fields ?? [])
                    .map((f) => `      <dt>${escapeString(f.label)}</dt>\n      <dd class="type">${escapeString(f.type)}</dd>`)
                    .join('\n')}\n    </dl>`;
            case 'actions':
                return `    <div class="block-actions">\n      <em>${escapeString(block.operation ?? '')}</em>\n    </div>`;
            case 'empty-state':
 ...
 */
function generateStarlightContentConfig() {
 ...
 */
function generateStarlightContentEntry(page) {
    const sections = [];
    for (const block of page.blocks ?? []) {
        const heading = block.description ?? block.name;
 ...
 * is wired in.
 */
function generateStarlightSidebar(pages) {
    const items = pages
